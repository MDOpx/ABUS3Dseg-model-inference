...
def load_case_from_list_of_files(data_files, seg_file=None, manual_resize=None):
    ...
    if manual_resize:
        if len(data_itk) > 1:
            raise NotImplementedError('Not Implemented resizing data for multimodal')
        
        from skimage.transform import resize

        new_size = manual_resize
        rawdata = sitk.GetArrayFromImage(data_itk[0])
        resized = resize(rawdata, reversed(new_size))
        data_itk[0] = sitk.GetImageFromArray(resized)
    ...
    if seg_file is not None:
        ...
        if manual_resize:
            rawdata = sitk.GetArrayFromImage(seg_itk)
            resized = resize(rawdata, reversed(new_size), order=0).astype(np.uint8)
            seg_itk = sitk.GetImageFromArray(resized)
...
class ImageCropper(object):
    ...
    @staticmethod
    def crop_from_list_of_files(data_files, seg_file=None, manual_resize=None):
        data, seg, properties = load_case_from_list_of_files(data_files, seg_file, manual_resize=manual_resize)
        return ImageCropper.crop(data, properties, seg)

    def load_crop_save(self, case, case_identifier, overwrite_existing=False, manual_resize=None):
        try:
            ...
            if overwrite_existing \
                ...
                data, seg, properties = self.crop_from_list_of_files(case[:-1], case[-1], manual_resize)
    ...
    def run_cropping(self, list_of_files, overwrite_existing=False, output_folder=None, manual_resize=None):
        """
        ...
        for j, case in enumerate(list_of_files):
            ...
            list_of_args.append((case, case_identifier, overwrite_existing, manual_resize))
