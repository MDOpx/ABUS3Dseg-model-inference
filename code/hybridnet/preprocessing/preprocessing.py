...
from hybridnet.configuration import default_num_threads, RESAMPLING_SEPARATE_Z_ANISO_THRESHOLD
from hybridnet.preprocessing.cropping import get_case_identifier_from_npz, ImageCropper
...
class GenericPreprocessor(object):
    ...
    def resample_and_normalize(self, data, target_spacing, properties, seg=None, force_separate_z=None, background_value=0):
        """
        ...
        for c in range(len(data)):
            ...
            if scheme == "CT":
                ...
                if use_nonzero_mask[c]:
                    ...
                    data[c][mask == 0] = background_value
                else:
                    mn = data[c].mean()
                    std = data[c].std()
    ...
    def preprocess_test_case(self, data_files, target_spacing, seg_file=None, force_separate_z=None, manual_resize=None):
        data, seg, properties = ImageCropper.crop_from_list_of_files(data_files, seg_file, manual_resize=manual_resize)
    ...
    def _run_internal(self, target_spacing, case_identifier, output_folder_stage, cropped_output_dir, force_separate_z,
        ...
        data, seg, properties = self.resample_and_normalize(data, target_spacing,
                                                            properties, seg, force_separate_z, background_value=background_value)
    ...
    def run(self, target_spacings, input_folder_with_cropped_npz, output_folder, data_identifier,
            num_threads=default_num_threads, force_separate_z=None, background_value=0):
        """
        ...
        for i in range(num_stages):
            ...
            for j, case in enumerate(list_of_cropped_npz_files):
                ...
                args = spacing, case_identifier, output_folder_stage, input_folder_with_cropped_npz, force_separate_z, all_classes, background_value
