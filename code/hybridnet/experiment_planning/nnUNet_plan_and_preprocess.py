...
from hybridnet.experiment_planning.DatasetAnalyzer import DatasetAnalyzer
from hybridnet.experiment_planning.utils import crop
from hybridnet.paths import *
import shutil
from hybridnet.utilities.task_name_id_conversion import convert_id_to_task_name
from hybridnet.preprocessing.sanity_checks import verify_dataset_integrity
from hybridnet.training.model_restore import recursive_find_python_class
...
def main():
    ...
    parser.add_argument("-tl", type=int, required=False, default=min(os.cpu_count(), int(os.environ['nnUNet_num_proc'])),
                        help="Number of processes used for preprocessing the low resolution data for the 3D low "
                             ...
                             "RAM")
    parser.add_argument("-tf", type=int, required=False, default=min(os.cpu_count(), int(os.environ['nnUNet_num_proc'])),
    ...
    parser.add_argument("--manual_resize", type=str, required=False, default=None),
    parser.add_argument("--background_value", type=int, required=False, default=0),
    parser.add_argument("--force_use_nonzero_mask", required=False, default=False, action="store_true"),

    args = parser.parse_args()
    if args.manual_resize:
        args.manual_resize = tuple([int(i) for i in args.manual_resize.split(',')])
    ...
    for i in task_ids:
        ...
        crop(task_name, False, tf, manual_resize=args.manual_resize)

        tasks.append(task_name)

    search_in = join(hybridnet.__path__[0], "experiment_planning")

    if planner_name3d is not None:
        planner_3d = recursive_find_python_class([search_in], planner_name3d, current_module="hybridnet.experiment_planning")
        if planner_3d is None:
            raise RuntimeError("Could not find the Planner class %s. Make sure it is located somewhere in "
                               "hybridnet.experiment_planning" % planner_name3d)
    else:
        planner_3d = None

    if planner_name2d is not None:
        planner_2d = recursive_find_python_class([search_in], planner_name2d, current_module="hybridnet.experiment_planning")
        if planner_2d is None:
            raise RuntimeError("Could not find the Planner class %s. Make sure it is located somewhere in "
                               "hybridnet.experiment_planning" % planner_name2d)
    else:
        planner_2d = None
    ...
    for t in tasks:
        ...
        if planner_3d is not None:
            ...
            if not dont_run_preprocessing:  # double negative, yooo
                exp_planner.run_preprocessing(threads, background_value=args.background_value, force_use_nonzero_mask=args.force_use_nonzero_mask)
