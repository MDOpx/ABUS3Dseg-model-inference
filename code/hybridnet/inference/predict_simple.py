...
from hybridnet.inference.predict import predict_from_folder
from hybridnet.paths import default_plans_identifier, network_training_output_dir, default_cascade_trainer, default_trainer
...
def main():
    ...
    parser.add_argument("--num_threads_preprocessing", required=False, default=min(os.cpu_count(), int(os.environ['nnUNet_num_proc'])), type=int, help=
    "Determines many background processes will be used for data preprocessing. Reduce this if you "
    "run into out of memory (RAM) problems. Default: 6")

    parser.add_argument("--num_threads_nifti_save", required=False, default=min(os.cpu_count(), int(os.environ['nnUNet_num_proc'])), type=int, help=
    ...
    parser.add_argument("--pp_mode", default='normal', choices=['normal', 'force', 'disabled', 'selective-0.5'], required=False)
    ...
    parser.add_argument("--nrrd_files", action='store_true', default=False)
    ...
    if isinstance(folds, list):
        if folds[0] == 'all' and len(folds) == 1:
            pass
        elif isinstance(folds[0], str) and len(folds) == 1:
            folds = [int(i) for i in folds[0].split(',')]
        else:
    ...
    predict_from_folder(model_folder_name, input_folder, output_folder, folds, save_npz, num_threads_preprocessing,
                        ...
                        manual_patch_size=args.manual_patch_size, model_arch=args.model_arch, encoder_module=args.encoder_module,
                        decoder_module=args.decoder_module, legacy_model=args.legacy_model, pp_mode=args.pp_mode, 
                        swa=args.swa, cascade_gap=args.cascade_gap, cls_classes=args.cls_classes, manual_resize=args.manual_resize,
                        nrrd_files=args.nrrd_files
                        )
    end = time()
