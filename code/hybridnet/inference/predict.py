...
from hybridnet.inference.segmentation_export import save_segmentation_nifti_from_softmax, save_segmentation_nifti
...
from hybridnet.postprocessing.connected_components import load_remove_save, load_postprocessing
from hybridnet.training.model_restore import load_model_and_checkpoint_files
from hybridnet.training.network_training.nnUNetTrainer import nnUNetTrainer
from hybridnet.utilities.one_hot_encoding import to_one_hot

from medcam import medcam

def preprocess_save_to_queue(preprocess_fn, q, list_of_lists, output_files, segs_from_prev_stage, classes,
                             transpose_forward, manual_resize=None):
    # suppress output
    ...
    for i, l in enumerate(list_of_lists):
        try:
            ...
            d, _, dct = preprocess_fn(l, manual_resize)
...
def preprocess_multithreaded(trainer, list_of_lists, output_files, num_processes=2, segs_from_prev_stage=None, manual_resize=None):
    ...
    for i in range(num_processes):
        pr = Process(target=preprocess_save_to_queue, args=(trainer.preprocess_patient, q,
                                                            ...
                                                            classes, trainer.plans['transpose_forward'], manual_resize))
        pr.start()
        processes.append(pr)
...
def predict_cases(model, list_of_lists, output_filenames, folds, save_npz, num_threads_preprocessing,
                  ...
                  manual_patch_size=None, model_arch=None, encoder_module=None, decoder_module=None, legacy_model=False, 
                  pp_mode='normal', cascade_gap=False, manual_device=None, swa=False, cls_classes=False,
    ...
    trainer, params = load_model_and_checkpoint_files(model, folds, mixed_precision=mixed_precision,
        checkpoint_name=checkpoint_name, manual_patch_size=manual_patch_size, model_arch=model_arch, encoder_module=encoder_module, decoder_module=decoder_module,
        legacy_model=legacy_model, cascade_gap=cascade_gap, manual_device=manual_device, swa=swa, cls_classes=cls_classes,
    ...
    if camlayer != None:
        trainer.network = medcam.inject(trainer.network, label=1, replace=True, backend="gcam", layer=camlayer) 
    ...
    for preprocessed in preprocessing:
        ...
        softmax = trainer.predict_preprocessed_data_return_seg_and_softmax(
            ...
            mixed_precision=mixed_precision, is_grad=camlayer!=None)[1]
        ...
        results.append(pool.starmap_async(save_segmentation_nifti_from_softmax,
                                          ((softmax, output_filename, dct, interpolation_order, region_class_order,
                                            None, None,
                                            npz_file, None, force_separate_z, interpolation_order_z, True, camlayer),)
                                          ))
    ...
    if not pp_mode == 'disabled':
        print("pp_mode", pp_mode)
        results = []
        pp_paths = sorted(glob(join(model, '**', 'postprocessing.json'), recursive=True))
        pp_file = pp_paths[0]
        if isfile(pp_file):
            ...
            elif 'selective' in pp_mode:
                raise NotImplementedError
                pp_threshold = float(pp_mode.split('-')[1])
                for_which_classes = [[1]]
                results.append(pool.starmap_async(load_remove_save,
                                                zip(output_filenames, output_filenames,
                                                    [for_which_classes] * len(output_filenames),
                                                    [min_valid_obj_size] * len(output_filenames),
                                                    [pp_threshold] * len(output_filenames),
                                                    )))
            
            else: raise KeyError(f"Unknown pp_mode: {pp_mode}")
            _ = [i.get() for i in results]
        else:
...
def check_input_folder_and_return_caseIDs(input_folder, expected_num_modalities, nrrd_files=False):
    print("This model expects %d input modalities for each image" % expected_num_modalities)
    if nrrd_files:
        files = subfiles(input_folder, suffix=".nrrd", join=False, sort=True)
...
def predict_from_folder(model: str, input_folder: str, output_folder: str, folds: Union[Tuple[int], List[int]],
                        ...
                        segmentation_export_kwargs: dict = None,
                        manual_patch_size=None, model_arch=None, encoder_module=None, decoder_module=None,
                        legacy_model=False, pp_mode='normal', swa=False, cascade_gap=False, cls_classes=False, manual_resize=None, nrrd_files=False,
                        attention_module=None, iba_estimate_loop=10000, cbammode=None, apply_skips=None, 
                        depth=[0,0,0,0,0,0], num_heads=[0,0,0,0,0,0], reduce_size=[0,0,0,0,0,0], projection=[[0],[0],[0],[0],[0],[0]],
                        camlayer = None):
    """
    ...
    assert isfile(join(model, "plans.pkl")), f"Folder with saved model weights must contain a plans.pkl file in {model}"
    expected_num_modalities = load_pickle(join(model, "plans.pkl"))['num_modalities']

    # check input folder integrity
    case_ids = check_input_folder_and_return_caseIDs(input_folder, expected_num_modalities, nrrd_files=nrrd_files)

    output_files = [join(output_folder, i + ".nii.gz") for i in case_ids]
    all_files = subfiles(input_folder, suffix=".nrrd" if nrrd_files else ".nii.gz", join=False, sort=True)
    list_of_lists = [[join(input_folder, i) for i in all_files if i[:len(j)].startswith(j) and
                      len(i) == (len(j) + (5 if nrrd_files else 12))] for j in case_ids]
    ...
    if mode == "normal":
        ...
        if model_arch == 'MultitaskAGIBA_NI':
            return predict_cases(model, list_of_lists[part_id::num_parts], output_files[part_id::num_parts], folds,
                                ...
                                manual_patch_size=manual_patch_size,
                                model_arch=model_arch, encoder_module=encoder_module, decoder_module=decoder_module, legacy_model=legacy_model,
                                pp_mode=pp_mode, cascade_gap=cascade_gap, swa=swa, cls_classes=cls_classes,
                                attention_module=attention_module, iba_estimate_loop=iba_estimate_loop, cbammode=cbammode, apply_skips=apply_skips, 
                                depth=depth, num_heads=num_heads, reduce_size=reduce_size, projection=projection,
                                camlayer=camlayer)
        else:
            return predict_cases(model, list_of_lists[part_id::num_parts], output_files[part_id::num_parts], folds,
                                save_npz, num_threads_preprocessing, num_threads_nifti_save, lowres_segmentations, tta,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cam_layer", type = str, default=None, help="List: [encoder.0, encoder.1, encoder.2, encoder.3, encoder.4, bottleneck, seg_decoder.decoder.0, seg_decoder.decoder.1, seg_decoder.decoder.2, seg_decoder.decoder.3, seg_decoder.decoder.4]")
    ...
    predict_from_folder(model, input_folder, output_folder, folds, save_npz, num_threads_preprocessing,
                        ...
                        model_arch=args.model_arch, encoder_module=args.encoder_module, decoder_module=args.decoder_module,
                        attention_module=args.attention_module, iba_estimate_loop=args.iba_estimate_loop, cbammode=args.cbammode, apply_skips=args.apply_skips, 
                        depth=args.depth, num_heads=args.num_heads, reduce_size=args.reduce_size, projection=args.projection,
                        camlayer=args.cam_layer)
