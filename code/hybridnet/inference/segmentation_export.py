...
from hybridnet.preprocessing.preprocessing import get_lowres_axis, get_do_separate_z, resample_data_or_seg
...
def save_segmentation_nifti_from_softmax(segmentation_softmax: Union[str, np.ndarray], out_fname: str,
                                         ...
                                         interpolation_order_z: int = 0, verbose: bool = True, camlayer: str = None):
    """
    ...
    :return:
    """
    is_grad = camlayer!=None
    if is_grad:
        if resampled_npz_fname is not None:
            os.makedirs(f"{resampled_npz_fname.split('/')[-2]}/{camlayer}", exist_ok= True)
            resampled_npz_fname = f"{resampled_npz_fname.split('/')[-2]}/{camlayer}/{resampled_npz_fname.split('/')[-1]}"
        os.makedirs(f"{out_fname.split('/')[-2]}/{camlayer}", exist_ok= True)
        out_fname = f"{out_fname.split('/')[-2]}/{camlayer}/{out_fname.split('/')[-1]}"
