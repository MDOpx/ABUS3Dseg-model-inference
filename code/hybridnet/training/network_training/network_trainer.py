...
class NetworkTrainer(object):
    def __init__(self, deterministic=True, fp16=False, lr_update_off=False):
        """
        ...
        self.lr_update_off = lr_update_off
    ...
    def load_checkpoint_ram(self, checkpoint, train=True):
        ...
        try:
            self.network.load_state_dict(new_state_dict, strict=False if self.swa else True)
        except: # Old model compatibility
            ...
            self.network.load_state_dict(converted_dict)
        
        if self.swa and not train:
            _do_ds = self.network.do_ds
            _predict_3D = self.network.predict_3D
            self.network = torch.optim.swa_utils.AveragedModel(self.network)
            self.network.do_ds = _do_ds
            self.network.predict_3D = _predict_3D
    ...
    def run_training(self):
        ...
        if self.swa:
            self.network_swa = torch.optim.swa_utils.AveragedModel(self.network).to('cuda')
            self.scheduler_swa = torch.optim.swa_utils.SWALR(self.optimizer, swa_lr=self.swa_lr)
        
        self.network.best_f1 = 0.
        self.network.best_auc = 0.
        ...
        while self.epoch < self.max_num_epochs:
            ...
            # train one epoch
            self.network.train()
            self.network.current_epoch = self.epoch
            ...
            self.all_tr_losses.append(np.nanmean(train_losses_epoch)) #YCH edit
            self.print_to_log_file("train loss : %.4f" % self.all_tr_losses[-1])
            if math.isnan(self.all_tr_losses[-1]): #YCH Added
                sys.exit("ERROR: loss is nan")

            if self.swa and self.epoch > self.swa_start:
                self.network_swa.update_parameters(self.network)
                self.scheduler_swa.step()
            ...
            with torch.no_grad():
                ...
                for b in range(self.num_val_batches_per_epoch):
                    ...
                    val_losses.append(l)
                self.all_val_losses.append(np.nanmean(val_losses)) #YCH edit
                ...
                if self.also_val_in_tr_mode:
                    ...
                    for b in range(self.num_val_batches_per_epoch):
                        ...
                        val_losses.append(l)
                    self.all_val_losses_tr_mode.append(np.nanmean(val_losses)) #YCH edit
    ...
    def manage_patience(self):
        ...
        if self.patience is not None:
            ...
            if self.epoch%100 == 0: #YCH ADDED
                self.save_checkpoint(join(self.output_folder, f"model_epoch{self.epoch:04d}.model"))
