...
from hybridnet.training.dataloading.dataset_loading import load_dataset, DataLoader3D, DataLoader2D, unpack_dataset
from hybridnet.training.loss_functions.dice_loss import DC_and_CE_loss
from hybridnet.training.network_training.network_trainer import NetworkTrainer
...
class nnUNetTrainer(NetworkTrainer):
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
                 unpack_data=True, deterministic=True, fp16=False, lr_update_off=False):
        """
        :param deterministic:
        ...
        """
        super(nnUNetTrainer, self).__init__(deterministic, fp16, lr_update_off)
    ...
    def get_basic_generators(self, manual_setting, positive_size, negative_size):
        self.load_dataset()
        self.do_split()
        ...
        if self.threeD:
            dl_tr = DataLoader3D(self.dataset_tr, self.basic_generator_patch_size, self.patch_size, self.batch_size,
                                 ...
                                 pad_mode="constant", pad_sides=self.pad_all_sides, memmap_mode='r', manual_setting=manual_setting, positive_size=positive_size, negative_size=negative_size)
    ...
    def preprocess_patient(self, input_files, manual_resize=None):
        """
        ...
        preprocessor_class = recursive_find_python_class([join(hybridnet.__path__[0], "preprocessing")],
                                                         preprocessor_name,
                                                         current_module="hybridnet.preprocessing")
        assert preprocessor_class is not None, "Could not find preprocessor %s in hybridnet.preprocessing" % \
                                               preprocessor_name
        ...
        d, s, properties = preprocessor.preprocess_test_case(input_files,
                                                             self.plans['plans_per_stage'][self.stage][
                                                                 'current_spacing'], manual_resize=manual_resize)
        return d, s, properties

    def preprocess_predict_nifti(self, input_files: List[str], output_file: str = None,
                                 softmax_ouput_file: str = None, mixed_precision: bool = True, is_grad: bool = False) -> None:
        """
        ...
        pred = self.predict_preprocessed_data_return_seg_and_softmax(d, do_mirroring=self.data_aug_params["do_mirror"],
                                                                     ...
                                                                     mixed_precision=mixed_precision, is_grad=is_grad)[1]
