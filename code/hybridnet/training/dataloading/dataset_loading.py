...
from hybridnet.paths import preprocessing_output_dir
...
class DataLoader3D(SlimDataLoaderBase):
    def __init__(self, data, patch_size, final_patch_size, batch_size, has_prev_stage=False,
                 ...
                 pad_sides=None, manual_setting=False, positive_size=1, negative_size=1):
        """
        ...
        if manual_setting:
            self.patch_select = [positive_size, negative_size]
        else:
    ...
    def determine_shapes(self):
        ...
        if self.patch_select is not None:
            self.modified_batch_size = self.patch_select[0]+self.patch_select[1]
            #self.modified_batch_size = 4
            if self.modified_batch_size < 1: self.modified_batch_size = 1
            
            data_shape = (self.modified_batch_size, num_color_channels, *self.patch_size)
            seg_shape = (self.modified_batch_size, num_seg, *self.patch_size)
