...
def restore_model(pkl_file, checkpoint=None, train=False, fp16=None, manual_patch_size=None, model_arch=None, encoder_module=None, decoder_module=None, legacy_model=False, cascade_gap=False, manual_device=None, swa=False, cls_classes=False,
    ...
    hybridnet.trainig.network_training for the file that contains the trainer and instantiate it with the arguments saved in the pkl file. If checkpoint
    ...
    init = info['init']
    name = info['name']
    search_in = join(hybridnet.__path__[0], "training", "network_training")
    tr = recursive_find_python_class([search_in], name, current_module="hybridnet.training.network_training")
    ...
    if tr is None:
        raise RuntimeError("Could not find the model trainer specified in checkpoint in hybridnet.trainig.network_training. If it "
                           "is not located there, please move it or change the code of restore_model. Your model "
                           "trainer can be located in any directory within hybridnet.trainig.network_training (search is recursive)."
    ...
    """if len(init) == 7:
        print("warning: this model seems to have been saved with a previous version of hybridnet. Attempting to load it "
...
def load_model_and_checkpoint_files(folder, folds=None, mixed_precision=None, checkpoint_name="model_best", manual_patch_size=None,
    ...
    from hybridnet.network_architecture.static_UNet import Conv3dBlock, NIConv3dBlock
    from hybridnet.network_architecture.multitask_DNA import MultitaskAGIBA_NI
    #DEL
    #from hybridnet.network_architecture.static_UNet import Static_UNet, Conv3dBlock#, Conv3dVBlock, ResConv3dBlock, ResConv3dBlockRev3, Dense3dBlock_CBR, ResConv3dTFBlock, NIConv3dBlock
    # from hybridnet.network_architecture.multitask import Multitask
    # from hybridnet.network_architecture.multitask_6L import Multitask6L
    # from hybridnet.network_architecture.multitask_AG import MultitaskAG
    # from hybridnet.network_architecture.multitask_CLS import MultitaskCLS
    # from hybridnet.network_architecture.multitask_DET import MultitaskDET
